...
class MetadataV2 {
    ...
    public validateAuthenticationAlgorithm(): boolean {
        const minAlgorithm = 1;
        const maxAlgorithm = 18;

        return this.authenticationAlgorithm >= minAlgorithm && this.authenticationAlgorithm <= maxAlgorithm;
    }
    ...
    public validatePublicKeyAlgAndEncoding():  boolean {
        const minEncoding = 256;
        const maxEncoding = 260;

        return this.publicKeyAlgAndEncoding >= minEncoding && this.publicKeyAlgAndEncoding <= maxEncoding;
    }
    ...
    public validatePublicKeyAlgAndEncodings(): boolean {
        ...
        for (const encoding of this.publicKeyAlgAndEncodings) {
            if (encoding < minEncoding || encoding > maxEncoding) {
                return false;
            }
        }
        return true;
    ...
    public validateAttestationTypes(): boolean {
        let dim = this.attestationTypes.length;
        if(dim != 0) {
            const minType = 15879;
            const maxType = 15882;
            for(const type of this.attestationTypes) {
    ...
    public validateKeyProtection(): boolean {
        const val = [1,2,4,8,16];
        return val.includes(this.keyProtection);
    } 
    ...
    public validateMatcherProtection(): boolean {
        let valideProtection = [1,2,4];
        return valideProtection.includes(this.matcherProtection);
    }
    ...
    public validateEcdaaTrustAnchors(): boolean {
        if(this.getAttestationTypes().includes(15881)) {
